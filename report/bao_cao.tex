...
\textbf{Nhược điểm.}
Đổi lại, mô hình phải học đồng thời ánh xạ từ cùng một biểu diễn lịch sử sang nhiều horizon khác nhau. Các dự báo gần và xa có độ khó và đặc điểm thống kê khác nhau, trong khi chúng vẫn chia sẻ cùng một Encoder GRU. Vì vậy, biểu diễn $h_w$ phải chứa thông tin hữu ích cho toàn bộ khoảng dự báo, thay vì chỉ tối ưu riêng cho bước $t+1$. Trong một số trường hợp, điều này có thể làm giảm nhẹ hiệu quả tại horizon rất gần so với một mô hình được huấn luyện chuyên biệt cho one-step forecasting.

Ngoài ra, kiến trúc đầu ra phụ thuộc trực tiếp vào giá trị $H$. Chẳng hạn, một mô hình được huấn luyện với $H=6$ sẽ có lớp đầu ra $\text{Linear}(d_h,6)$ và sinh trực tiếp sáu giá trị tương ứng với các thời điểm từ $t+1$ đến $t+6$. Nếu muốn thay đổi horizon dự báo sang $H=12$, cần thay đổi kích thước lớp đầu ra thành $\text{Linear}(d_h,12)$ và thông thường phải huấn luyện lại mô hình.
