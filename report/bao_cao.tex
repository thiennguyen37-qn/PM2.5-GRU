...
Ô nhiễm không khí, đặc biệt là bụi mịn PM2.5 (các hạt vật chất có đường kính khí động học nhỏ hơn 2.5 micromet), là một trong những mối đe dọa hàng đầu đối với sức khỏe cộng đồng tại các đô thị. Do kích thước siêu nhỏ, PM2.5 có khả năng xâm nhập sâu vào phổi và hệ tuần hoàn, làm gia tăng nguy cơ mắc các bệnh hô hấp và tim mạch. Việc dự báo sớm nồng độ PM2.5 theo thời gian thực có ý nghĩa quan trọng trong việc cảnh báo cộng đồng và hỗ trợ ra quyết định quản lý môi trường đô thị.

Trong tiểu luận này, bài toán được đặt ra là \textbf{dự báo chuỗi thời gian (time series forecasting)}: dự báo nồng độ PM2.5 tại thời điểm kế tiếp (1 giờ sau) dựa trên lịch sử quan trắc đa biến (PM2.5, các chỉ số chất lượng không khí liên quan, và các đặc trưng thời gian) trong một cửa sổ quan sát trong quá khứ. Mô hình học sâu được lựa chọn để giải quyết bài toán là mạng \textbf{GRU (Gated Recurrent Unit)} — một biến thể của mạng nơ-ron hồi quy (Recurrent Neural Network) được thiết kế để nắm bắt các phụ thuộc dài hạn trong dữ liệu tuần tự.

Bộ dữ liệu sử dụng được thu thập từ \textbf{Open-Meteo Air Quality API} tại tọa độ $13.800003^\circ$N, $109.20001^\circ$E — tương ứng vị trí thành phố Quy Nhơn, tỉnh Gia Lai. Dữ liệu được ghi nhận theo giờ, liên tục từ \texttt{2022-08-04 07:00} đến \texttt{2026-07-26 23:00}, gồm 34.865 quan sát.

Thông qua bài tiểu luận này, em thực hành đầy đủ quy trình xây dựng một mô hình học sâu cho bài toán dự báo chuỗi thời gian, bao gồm:

\begin{itemize}
    \item Thu thập và làm sạch dữ liệu
    \item Phân tích khám phá dữ liệu (EDA) chuyên biệt cho chuỗi thời gian: tính dừng, tính chu kỳ, xu hướng
    \item Tiền xử lý dữ liệu và feature engineering (mã hóa chu kỳ thời gian, chuẩn hóa, sliding window)
    \item Xây dựng và tinh chỉnh siêu tham số cho mô hình GRU
    \item Đánh giá mô hình trên tập kiểm tra và trên một bộ dữ liệu hoàn toàn mới, được thu thập sau này (out-of-sample thực sự)
\end{itemize}

Báo cáo sẽ trình bày chi tiết từng bước thực hiện, phân tích kết quả thu được và đưa ra nhận xét về hiệu quả của mô hình.

% ==================== 2. CƠ SỞ LÝ THUYẾT ====================
\section{Cơ sở lý thuyết}

\subsection{Bài toán dự báo chuỗi thời gian bằng học sâu}
...
trong đó $x_i \in \mathbb{R}^n$ là vector $n$ đặc trưng tại thời điểm $i$ (bao gồm PM2.5 và các chỉ số liên quan). Trong tiểu luận này, $h = 1$ (dự báo 1 giờ tới) và $w$ được xác định thông qua tinh chỉnh siêu tham số ở Mục~4.
...
\begin{table}[H]
\centering
\caption{So sánh RNN, LSTM và GRU}
\begin{tabular}{lccc}
\toprule
Tiêu chí & RNN & LSTM & GRU \\
\midrule
Số cổng (gate) & 0 & 3 (quên, vào, ra) & 2 (đặt lại, cập nhật) \\
Trạng thái riêng biệt & Không & Cell state $C_t$ + $h_t$ & Chỉ $h_t$ \\
Số tham số (tương đối) & Thấp nhất & Cao nhất & Trung bình ($\approx$75\% LSTM) \\
Khả năng chống vanishing gradient & Kém & Tốt & Tốt \\
Tốc độ huấn luyện & Nhanh nhất & Chậm nhất & Nhanh hơn LSTM \\
\bottomrule
\end{tabular}
\end{table}

Nhờ số tham số ít hơn LSTM (không cần cổng ra và cell state riêng) trong khi vẫn giữ được khả năng nắm bắt phụ thuộc dài hạn tương đương trong nhiều bài toán thực nghiệm, GRU được lựa chọn cho bài toán dự báo PM2.5 với chuỗi dữ liệu có độ dài vừa phải (khoảng 35 nghìn điểm) — nơi tốc độ huấn luyện nhanh hơn có lợi thế rõ rệt khi cần chạy Grid Search nhiều cấu hình (Mục~4.2) mà không đánh đổi đáng kể về chất lượng mô hình.

\subsection{Hàm mất mát và quá trình huấn luyện}
...
\subsubsection{Tương quan giữa các chỉ số chất lượng không khí}
...
\subsubsection{Xu hướng và phân phối theo thời gian}
...
\subsubsection{Tính dừng và xu hướng dài hạn}
...
\subsubsection{Tính chu kỳ (Seasonality)}
...
Biểu đồ ACF (Hình~\ref{fig:acfpacf}) thể hiện các đỉnh lặp lại rõ rệt tại lag 24, 48, 72, 96... giờ — xác nhận \textbf{chu kỳ ngày (24 giờ)} là chu kỳ chi phối hành vi ngắn hạn của PM2.5, phù hợp với quy luật sinh hoạt/giao thông đô thị (giờ cao điểm) lặp lại mỗi ngày. Đây là căn cứ quan trọng để lựa chọn kích thước cửa sổ đầu vào (window size) cho mô hình ở Mục~4.
...
Dựa trên kết quả phân tích tương quan ở Mục~2.2.1 (Correlation Heatmap $|r| < 0.1$, và Cross-Correlation Function ở mọi độ trễ đều $< 0.16$), ba biến \textbf{Dust}, \textbf{UV Index}, \textbf{UV Index Clear Sky} được loại bỏ khỏi tập đặc trưng đầu vào nhằm giảm nhiễu và số chiều dữ liệu.
...
\subsection{Sliding Window: Tạo cặp $(X, y)$}
